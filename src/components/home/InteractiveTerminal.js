...
const ROUTES = { about: '/about', projects: '/projects', contact: '/contact', home: '/', '~': '/', admin: '/admin' };
...
const HELP = {
    ...
    cd: { usage: 'cd <page>', detail: 'Goes to /about, /projects, /contact, or /home.' },
...
export default function InteractiveTerminal({ active, onExit }) {
    ...
    const runCommand = (raw) => {
        ...
        switch (cmd) {
            case 'help':
                print(trimmed, (
                    <span className={styles.helpList}>
                        <span className={styles.helpIntro}>commands:</span>
                        {Object.entries(HELP).map(([ key, { usage, detail } ]) => (
                            <span key={key} className={styles.helpRow}>
                                <span className={styles.helpUsage}>{usage}</span>
                                <span className={styles.helpDetail}>{detail}</span>
                            </span>
                        ))}
                    </span>
                ));
            ...
            <div className={styles.history}>
                {history.map((entry, i) => (
                    <div key={i} className={styles.entry}>
                        <p className={styles.line}><span className='prompt-symbol'>rafi@codes:~$</span> {entry.command}</p>
                        {entry.output && <p className={styles.output}>{entry.output}</p>}
                    </div>
                ))}
                <div className={styles.inputLine}>
                    <span className='prompt-symbol'>rafi@codes:~$ </span>
                    ...
                    <input ref={inputRef} className={styles.hiddenInput} value={input} onChange={(e) => { handleChange(e); syncCursor(e); }} onKeyDown={handleKeyDown} onKeyUp={syncCursor} onClick={syncCursor} onSelect={syncCursor} autoComplete='off' autoCapitalize='off' autoCorrect='off' spellCheck='false' inputMode='text' aria-label='Terminal command input'/>
                </div>
