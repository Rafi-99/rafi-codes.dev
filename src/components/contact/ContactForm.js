...
export default function ContactForm() {
    ...
    const handleSubmit = async (event) => {
        ...
        const serverResponse = await fetch('/api/contact', {
            ...
            body: JSON.stringify({ ...inputs, token: recaptchaToken }),
        });
        ...
        <TerminalWindow title='rafi@codes: ~/contact' className={styles.formContainer} variant='ambient'>
            <div className={styles.body}>
                ...
                <form action='/api/contact' method='POST' id='email-form' onSubmit={handleSubmit}>
                    ...
                    <input type='text' className='form-input' aria-label='Name' id='name' name='name' value={inputs.name || ''} onChange={handleChange} required autoComplete='off' />

                    <label htmlFor='email'><span className={styles.comment}>#</span> Email</label>
                    <input type='email' className='form-input' aria-label='Email' id='email' name='email' value={inputs.email || ''} onChange={handleChange} required autoComplete='off' />

                    <label htmlFor='message'><span className={styles.comment}>#</span> Message</label>
                    <textarea rows='5' form='email-form' className='form-input' aria-label='Message' id='message' name='message' value={inputs.message || ''} onChange={handleChange} required />

                    <button ref={button} type='submit' aria-label='Send Message' className='pushable accent'>
                        <span ref={label} className={styles.buttonLabel}>$ send --message</span>
                    </button>
                    ...
                    <small>
                        ...
                        <Link href='https://policies.google.com/privacy' rel='noopener noreferrer' target='_blank'>Privacy Policy</Link>{' '}
                        and{' '}
                        <Link href='https://policies.google.com/terms' rel='noopener noreferrer' target='_blank'>Terms of Service</Link>{' '}
                        apply.
                    </small>
                </form>
            </div>
